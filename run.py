...
import ROOT # needed
import os, sys, glob, sys, json, re
import logging
...
def draw_instack(variable, label='VBF', select=''):
    ...
    for proc in ordsam:
        ...
        if proc != 'Data':        
            tree.Project(
                ...
                formula,
                #cutflow.replace('weight','(weight/((puweight>0)*puweight + (puweight==0)*1))*%f' % treeinfo.get('kfactor',1.0) )
    ...
    for h in histos:
        if 'data' in h.GetName():
            ...
            h.Draw('hist,same')
    herrstat = drawStatErrorBand(hstack.GetStack().Last())
    ...
    #if not os.path.exists('plots/png'): os.makedirs('plots/png')
...
if __name__ == "__main__":
    ...
    for var in variables:
        ...
        #count = count + 1
        #
    #draw_instack('nvtx'    ,options.label,selection['title'])
    #draw_instack('cosdphi:=abs(cos(dijet_dipho_dphi))' ,options.label,selection['title'])
    #draw_instack('n_jets'  ,options.label,selection['title'])
    #raw_input('...')
